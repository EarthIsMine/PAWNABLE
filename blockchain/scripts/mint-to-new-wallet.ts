import { ethers } from "hardhat";

async function main() {
  const usdtAddress = "0x5FbDB2315678afecb367f032d93F642f64180aa3";
  const usdcAddress = "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0";

  // Hardhat 테스트 계정들
  const accounts = [
    "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", // Account #0
    "0x70997970C51812dc3A010C7d01b50e0d17dc79C8", // Account #1
    "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC", // Account #2
    "0x90F79bf6EB2c4f870365E785982E1f101E93b906", // Account #3
    "0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65", // Account #4
  ];

  console.log("🎁 새 지갑에 토큰 민팅 중...\n");

  const MockUSDT = await ethers.getContractFactory("MockUSDT");
  const usdt = MockUSDT.attach(usdtAddress);
  const usdc = MockUSDT.attach(usdcAddress);

  const amount = ethers.parseUnits("100000", 6); // 100,000 토큰

  // 각 계정에 민팅
  for (let i = 0; i < accounts.length; i++) {
    console.log(`\n📝 Account #${i} (${accounts[i]}):`);

    await usdt.mint(accounts[i], amount);
    console.log(`  ✅ USDT: ${ethers.formatUnits(amount, 6)} 민팅됨`);

    await usdc.mint(accounts[i], amount);
    console.log(`  ✅ USDC: ${ethers.formatUnits(amount, 6)} 민팅됨`);

    // 잔액 확인
    const usdtBalance = await usdt.balanceOf(accounts[i]);
    const usdcBalance = await usdc.balanceOf(accounts[i]);

    console.log(`  💰 USDT 잔액: ${ethers.formatUnits(usdtBalance, 6)}`);
    console.log(`  💰 USDC 잔액: ${ethers.formatUnits(usdcBalance, 6)}`);
  }

  console.log("\n✅ 완료!");
  console.log("\n메타마스크에서 사용할 Private Keys:");
  console.log("Account #0: 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80");
  console.log("Account #1: 0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d");
  console.log("Account #2: 0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a");
  console.log("Account #3: 0x7c852118294e51e653712a81e05800f419141751be58f605c371e15141b007a6");
  console.log("Account #4: 0x47e179ec197488593b187f80a00eb0da91f1b9d0b13f8733639f19c30a34926a");
}

main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });
