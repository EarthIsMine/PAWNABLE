...
> pnpm 모노레포이므로 의존성 설치는 저장소 루트(`/srv/PAWNABLE`)에서 한 번만 실행하는 것을 권장합니다.
...
`.env` 파일 수정:
```env
DATABASE_URL="postgresql://user:password@localhost:5432/pawnable?schema=public"
BASE_CHAIN_ID=103
...
# 스키마 동기화
pnpm --filter @pawnable/backend prisma:push

# 초기 토큰 시드
pnpm --filter @pawnable/backend seed:dev
```

### 4. 실행

```bash
pnpm --filter @pawnable/backend dev             # 개발 서버 (hot reload)
pnpm --filter @pawnable/backend build           # 빌드
pnpm --filter @pawnable/backend start           # 프로덕션
pnpm --filter @pawnable/backend prisma:studio   # DB GUI
```
    ...
    "loanRequests": [
      {
        ...
        "borrower": { "address": "0x..." },
        "collateralToken": { "symbol": "WLC", "decimals": 18 },
        "principalToken": { "symbol": "USDT", "decimals": 6 }
      }
    ],
    "total": 42,
    "limit": 50,
...
```json
{
  ...
  "collateralAmount": "1000000000000000000",
  "principalToken": "0x4046bd9ec8223c2a9354dc517b2d2d67b75cebfb",
  "principalAmount": "1000000000",
  "interestBps": 500,
  "durationSeconds": 604800,
  "createTxHash": "0xabcd...1234",
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `chainId` | `number` | 체인 ID (103 = WorldLand Mainnet) |
| `contractAddress` | `address` | 대출 컨트랙트 주소 |
| `onchainRequestId` | `string` | 온체인 requestId (uint256) |
| `borrower` | `address` | 차입자 지갑 주소 |
| `collateralToken` | `address` | 담보 토큰 주소 (`0x0000000000000000000000000000000000000000` = WLC) |
...
```json
{
  "success": true,
  "data": [
    { "chainId": 103, "address": "0x0000...0000", "symbol": "WLC", "decimals": 18, "isNative": true, "isAllowed": true },
    { "chainId": 103, "address": "0x4046...ebfb", "symbol": "USDT", "decimals": 6, "isNative": false, "isAllowed": true }
  ]
}
```
...
// USDT: "1000000" -> 1.0 USDT (decimals=6)
```

이자 계산:
```typescript
// interest = principal * interestBps / 10000
// 예: 1000 USDT * 500bps = 50 USDT (5%)
...
| `BASE_RPC_URL` | O | WorldLand RPC URL | - |
| `LOAN_CONTRACT_ADDRESS` | - | 대출 컨트랙트 주소 | `""` |
| `PORT` | - | 서버 포트 | `8080` |
| `NODE_ENV` | - | 환경 | `development` |
| `BASE_CHAIN_ID` | - | 체인 ID | `103` |
| `ENABLE_INDEXER` | - | 이벤트 인덱서 활성화 여부 | `false` |

---

## 시드 토큰

`pnpm --filter @pawnable/backend seed:dev` 실행 시 등록되는 토큰 (WorldLand Mainnet):

| 심볼 | 주소 | Decimals | Native |
|------|------|----------|--------|
| WLC | `0x0000...0000` | 18 | O |
| USDT | `0x4046bd9ec8223c2a9354dc517b2d2d67b75cebfb` | 6 | - |
